...
function launchBuilder(){
  ...
  +'._lb_ta{resize:none;min-height:70px;overflow:hidden}'
  ...
  +'._lb_entry[data-type="Character"]{border-left-color:#818cf8}'
  ...
  +'._lb_enum{font-size:.82rem;color:#f87171;font-weight:700;letter-spacing:.04em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:340px}'
  ...
  +'._lb_fbtn[data-type="Character"]._on{background:#4f46e5}'
  +'._lb_fbtn[data-type="Item"]._on{background:#059669}'
  ...
  var tConv=mkTab('Convert','conv',false);
  var tSaveLoad=mkTab('Save / Load','saveload',false);
  ...
  var nw=document.createElement('div');nw.style.marginBottom='14px';
  var nl=document.createElement('label');nl.className='_lb_lbl';nl.textContent='Lorebook Name';
  var ni=document.createElement('input');ni.type='text';ni.id='_lb_nm';ni.className='_lb_inp';ni.placeholder='My Lorebook';
  ...
  searchInp.style.cssText='flex:1;background:#111827;border:1px solid #374151;border-radius:6px;color:#e5e7eb;padding:6px 10px;font-size:.83rem;outline:none;font-family:system-ui,sans-serif';
  ...
  searchTopRow.appendChild(searchInp);searchTopRow.appendChild(searchMode);
  ...
  var groupBtn=document.createElement('button');groupBtn.className='_lb_btn _lb_btns';groupBtn.style.cssText='padding:3px 10px;font-size:.73rem;margin-left:6px';groupBtn.textContent='Group by type';
  var shiftHint=document.createElement('span');shiftHint.style.cssText='font-size:.68rem;color:#4b5563;margin-left:4px';shiftHint.textContent='Shift+click for multi';

  var activeFilters=new Set(['All']);
  ['All','Character','Item','PlotEvent','Location','Other'].forEach(function(type){
  ...
  var pConv=document.createElement('div');pConv.id='_lb_p_conv';pConv.className='_lb_pnl';
  ...
  pConv.appendChild(tplStrip);pConv.appendChild(upLabel);pConv.appendChild(dropZone);pConv.appendChild(parseErr);pConv.appendChild(previewDiv);

  var pSaveLoad=document.createElement('div');pSaveLoad.id='_lb_p_saveload';pSaveLoad.className='_lb_pnl';
  ...
  pSaveLoad.appendChild(exportBox);pSaveLoad.appendChild(loadBox);
  ...
  function mkDeskToggle(title,desc,key){
  ...
  bd.appendChild(pBuild);bd.appendChild(pConv);bd.appendChild(pSaveLoad);bd.appendChild(pSettings);
  ...
  var hotkeyHint=document.createElement('span');hotkeyHint.id='_lb_hotkey_hint';hotkeyHint.textContent='Alt+N — new entry';if(IS_MOBILE)hotkeyHint.style.display='none';
  var rfr=document.createElement('div');rfr.style.cssText='display:flex;align-items:center;gap:10px';
  ...
  ft.appendChild(clrBtn);ft.appendChild(importEntriesBtn);ft.appendChild(hotkeyHint);ft.appendChild(rfr);
    ...
    [['_tl','nw'],['_tr','ne'],['_bl','sw'],['_br','se']].forEach(function(cfg){
      ...
      c.addEventListener('mousedown',function(e){
        ...
        function onMove(e){
          ...
          bx.style.left=(cx-nw/2)+'px';bx.style.top=(cy-nh/2)+'px';bx.style.width=nw+'px';bx.style.height=nh+'px';
        }
        function onUp(){
  ...
  if(IS_MOBILE){document.body.appendChild(bx);}else{document.body.appendChild(bx);try{var _p=JSON.parse(localStorage.getItem('_lb_window_pos')||'null');if(_p&&_p.w>=480&&_p.h>=320){bx.style.left=_p.l+'px';bx.style.top=_p.t+'px';bx.style.width=_p.w+'px';bx.style.height=_p.h+'px';bx.style.borderRadius='8px';bx.style.boxShadow='0 8px 40px rgba(0,0,0,.6)';bx.style.border='1px solid #374151';}}catch(e){}}
  ...
  function mkEntry(data){
    ...
    var eStatWrap=document.createElement('span');eStatWrap.style.cssText='display:flex;gap:6px;align-items:center;flex-shrink:0;margin-left:8px;';
    ...
    ebtns.appendChild(colBtn);ebtns.appendChild(delBtn);
    eleft.appendChild(dragHandle);eleft.appendChild(typeDot);eleft.appendChild(enumEl);
    ehead.appendChild(eleft);ehead.appendChild(eStatWrap);ehead.appendChild(ebtns);
    function updStats(){
    ...
    var delimToggle=document.createElement('button');
    delimToggle.className='_lb_ebtn';
    delimToggle.style.cssText='flex-shrink:0;font-size:.72rem;padding:4px 8px;white-space:nowrap';
    function updDelimUI(){
      delimToggle.textContent=delim===','?', comma':'; semicolon';
      delimToggle.title=delim===','?'Switch to semicolon delimiter':'Switch back to comma delimiter';
      trigI.placeholder=delim===','?'keyword, another, etc.':'keyword; another; etc.';
    }
    updDelimUI();
    delimToggle.addEventListener('click',function(e){
      e.stopPropagation();var oldDelim=delim;delim=delim===','?';':',';
      var cur=trigI.value.split(oldDelim).map(function(t){return t.trim();}).filter(Boolean);
      trigI.value=cur.join(delim===','?', ':'; ');
      el._triggers=cur;updDelimUI();scheduleSave();
    });
    trigWrapInner.appendChild(trigI);trigWrapInner.appendChild(delimToggle);
    ...
    function autoGrow(){descTA.style.height='auto';descTA.style.height=Math.max(descTA._minH||70,descTA.scrollHeight)+'px';}
    ...
    dLbl.appendChild(dNote);descWrap.appendChild(dLbl);descWrap.appendChild(descTA);descWrap.appendChild(descTab);descWrap.appendChild(cc);

    var trigFieldWrap=document.createElement('div');
    var trigLbl=document.createElement('label');trigLbl.className='_lb_lbl';trigLbl.textContent='Trigger Keywords';

    if(window._lb_compact_triggers){
      trigFieldWrap.appendChild(trigLbl);trigFieldWrap.appendChild(trigOuter);
    } else {
      var tagWrap=document.createElement('div');tagWrap.appendChild(tagRow);tagWrap.appendChild(trigCounter);
      trigFieldWrap.appendChild(trigLbl);trigFieldWrap.appendChild(tagWrap);
    }
    ...
    var sugTray=document.createElement('div');sugTray.style.cssText='margin-top:6px';sugTray.style.display='none';
    ...
    descTA.addEventListener('input',function(){autoGrow();updCC();sugOffset=0;renderSugs();scheduleSave();});
  ...
  function addEntry(data){eDiv.appendChild(mkEntry(data));renumberEntries();scheduleSave();}
  ...
  function countMatches(){
    ...
    eDiv.querySelectorAll('._lb_entry').forEach(function(el){
      ...
      var dh=(d.description||'').match(re);if(dh)found+=dh.length;
  ...
  function loadState(state){
    document.getElementById('_lb_nm').value=state.name||'';
    eDiv.innerHTML='';ec=0;
  ...
  var panels={build:pBuild,conv:pConv,saveload:pSaveLoad,settings:pSettings};
  var tabs={build:tBuild,conv:tConv,saveload:tSaveLoad,settings:tSettings};
  ...
  tConv.addEventListener('click',function(e){e.stopPropagation();switchTab('conv');});
  tSaveLoad.addEventListener('click',function(e){e.stopPropagation();switchTab('saveload');});
  ...
  ni.addEventListener('input',scheduleSave);
  ...
  clrBtn.addEventListener('click',function(e){
    ...
    document.getElementById('_lb_nm').value='';eDiv.innerHTML='';ec=0;
  ...
  function hotkeyHandler(e){
    if(e.altKey&&e.key==='n'&&document.getElementById('_lb_ov')){
      e.preventDefault();
      switchTab('build');
      addEntry();
      var entries=eDiv.querySelectorAll('._lb_entry');
      var last=entries[entries.length-1];
      if(last){last.scrollIntoView({behavior:'smooth',block:'center'});var inp=last.querySelector('._lb_inp');if(inp)setTimeout(function(){inp.focus();},80);}
    }
  }
