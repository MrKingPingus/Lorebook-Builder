...
function launchBuilder(){
  ...
  +'._lb_ta{resize:none;min-height:70px;overflow:hidden;background:transparent!important;position:relative;z-index:1;line-height:1.5}'
  +'._lb_desc_hl_wrap{position:relative}'
  +'._lb_desc_hl{position:absolute;top:0;left:0;right:0;bottom:0;pointer-events:none;overflow:hidden;color:transparent;white-space:pre-wrap;word-wrap:break-word;word-break:break-word;box-sizing:border-box;padding:7px 10px;font-size:.87rem;font-family:system-ui,sans-serif;line-height:1.5;border:1px solid transparent;border-radius:6px;background:#111827;z-index:0}'
  ...
  colAllBtn.addEventListener('click',function(e){
    ...
    if(allCollapsed){
      eDiv.querySelectorAll('._lb_entry').forEach(function(el){
        el.classList.remove('_lb_collapsed');
        var cb=el.querySelector('._lb_ebtn');if(cb)cb.textContent='▲ Collapse';
      });
      colAllBtn.textContent='Collapse All';
    } else {
      eDiv.querySelectorAll('._lb_entry').forEach(function(el){
        el.classList.add('_lb_collapsed');
        var cb=el.querySelector('._lb_ebtn');if(cb)cb.textContent='▼ Expand';
      });
      colAllBtn.textContent='Expand All';
    }
  });
  ...
  rstBtn.addEventListener('click',function(e){e.stopPropagation();applyDefaultSize();});
  ...
  var hotkeyHint=document.createElement('span');hotkeyHint.id='_lb_hotkey_hint';hotkeyHint.textContent='Alt+'+(window._lb_hotkey_new||'n').toUpperCase()+' — new entry';if(IS_MOBILE)hotkeyHint.style.display='none';
  ...
  ft.appendChild(clrBtn);ft.appendChild(hotkeyHint);ft.appendChild(rfr);
  ...
  if(!IS_MOBILE){
    ...
    function applyDefaultSize(){
      ...
      try{localStorage.removeItem('_lb_window_pos');}catch(e){}
    }
  ...
  function mkEntry(data){
    ...
    function syncDescHl(q){
      var text=descTA.value;
      var safe=text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
      if(q&&text.toLowerCase().indexOf(q)!==-1){
        var esc=q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
        safe=safe.replace(new RegExp('('+esc+')','gi'),'<mark style="background:#fef08a;border-radius:2px">$1</mark>');
      }
    ...
    descTA.addEventListener('input',function(){autoGrow();updCC();sugOffset=0;renderSugs();scheduleSave();var curQ=searchInp.value.trim().toLowerCase();if(curQ)syncDescHl(curQ);});
  ...
  function applyFilter(){
    ...
    var MARK='background:#fef08a;color:#111827;border-radius:2px;padding:0 1px';
    function hlText(str){return str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(hlRe,'<mark style="'+MARK+'">$1</mark>');}
    eDiv.querySelectorAll('._lb_entry').forEach(function(el){
      ...
      var enumEl=el.querySelector('._lb_enum');
      if(enumEl){
        if(hlRe&&el.classList.contains('_lb_search_match')){
      ...
      // highlight trigger chip labels
      el.querySelectorAll('._lb_trig_lbl').forEach(function(lbl){
        var orig=lbl.dataset.origText||(lbl.dataset.origText=lbl.textContent);
        if(hlRe&&el.classList.contains('_lb_search_match')&&orig.toLowerCase().indexOf(q)!==-1){
          lbl.innerHTML=hlText(orig);
        } else {
          lbl.textContent=orig;
        }
      });
      // highlight description text via overlay
      if(el._syncDescHl){
        el._syncDescHl(hlRe&&el.classList.contains('_lb_search_match')?q:'');
      }
    });
    updMatchCount();
  }
  ...
  searchInp.addEventListener('input',function(e){e.stopPropagation();eDiv.querySelectorAll('._lb_enum[data-orig-text]').forEach(function(el){delete el.dataset.origText;});applyFilter();});
