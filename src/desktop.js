...
function launchBuilder(){
  ...
  +'#_lb_nm{background:transparent;border:none;border-bottom:1px solid #374151;color:#e5e7eb;padding:3px 6px;font-size:.85rem;outline:none;font-family:system-ui,sans-serif;flex:1;min-width:60px;max-width:200px;margin:0 12px}'
  ...
  var searchFilterBar=document.createElement('div');
  searchFilterBar.appendChild(searchRow);searchFilterBar.appendChild(filterBar);
  function applySearchPin(){searchFilterBar.style.cssText=window._lb_pin_search?'position:sticky;top:0;z-index:2;background:#1f2937;padding-bottom:8px':'margin-bottom:2px';}
  applySearchPin();
  pBuild.appendChild(restoreBanner);pBuild.appendChild(searchFilterBar);pBuild.appendChild(eDiv);pBuild.appendChild(aeBtn);
  ...
  try{var s_ps=localStorage.getItem('_lb_pin_search');window._lb_pin_search=(s_ps===null)?true:(s_ps==='1');}catch(e){window._lb_pin_search=true;}
  applySearchPin();
  ...
  setBox.appendChild(mkDeskToggle('Pin search bar','Keep the search and filter bar visible at the top while scrolling entries.','_lb_pin_search',applySearchPin));
